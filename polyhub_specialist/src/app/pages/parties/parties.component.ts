...
export class PartiesComponent implements OnInit {
  ...
  // rate modal (read-then-rate flow)
  ...
  openInfo(party: PartyForRatingResponse) {
    this.infoParty = party;
  }
  ...
  openRate(party: PartyForRatingResponse) {
    ...
    this.ratingPoint = {
      x: party.specEconomicAxis ?? 0,
      y: party.specSocialAxis ?? 0
    };
    this.ratingError = '';
  }

  closeRate() {
    this.ratingParty = null;
  }

  confirmRead() {
    this.hasReadParty = true;
  }
